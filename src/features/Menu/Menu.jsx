...
const Menu = () => {
  ...
  const totalQuantity = selectedProducts.reduce((acc, curr) => {
    return acc + curr.quantity;
  }, 0);

  console.log({ totalPrice, totalQuantity });

  // const ordering = (orderQuantity) => {
  //   console.log("test");
  //   axios
  //     .post("https://huge-socks-stop.loca.lt/carts", {addorder})
  //     .then((Response) => {
  ...
  return (
    <div className="min-h-screen p-4">
      ...
      <div className="flex overflow-x-auto overflow-hidden no-scrollbar mt-8">
        <div className="flex flex-nowrap justify-between gap-5 w-full">
          {kategori.map((kategori, index) => {
            return (
              <KategoriCard name={kategori.name} picture={kategori.picture} />
            );
          })}
        </div>
      </div>
      ...
      <div className="text-black font-medium text-xl mt-4">Special Price</div>
